...
describe("tenant edge cache (Cache API + gen keys)", () => {
  ...
  it("misses cold, hits warm, with s-maxage=3600 on the cached response", async () => {
    await setGen(1000);
    const r1 = await SELF.fetch("https://alice.nostrbook.net/");
  ...
  it("caches per path (post page and home are separate entries)", async () => {
    await setGen(1100);
    const post1 = await SELF.fetch("https://alice.nostrbook.net/hello-world");
    expect(post1.status).toBe(200);
    expect(post1.headers.get(CACHE_STATUS_HEADER)).toBe("miss");
    const post2 = await SELF.fetch("https://alice.nostrbook.net/hello-world");
  ...
  it("mirrorEvent's gen bump invalidates cached pages (new post appears)", async () => {
    ...
    expect(
      (await SELF.fetch("https://alice.nostrbook.net/")).headers.get(
        CACHE_STATUS_HEADER,
      ),
    ).toBe("hit");
  ...
  it("does not cache 404s", async () => {
    await setGen(1300);
    const r1 = await SELF.fetch("https://alice.nostrbook.net/nope");
    expect(r1.status).toBe(404);
    expect(r1.headers.get(CACHE_STATUS_HEADER)).toBeNull();
    const r2 = await SELF.fetch("https://alice.nostrbook.net/nope");
  ...
  it("keeps serving the cached page until the generation moves", async () => {
    ...
    await setGen(1401);
    const r3 = await SELF.fetch("https://alice.nostrbook.net/");
